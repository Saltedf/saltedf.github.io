...
6. builder.create_block()创建入口块
7.


** EntryIndex
EntryIndex 的作用是为每个基本块生成一个唯一的标识符，方便在 Cranelift 内部进行引用和操作。可以通过 FunctionBuilder::create_block() 方法创建一个新的基本块，并得到该基本块的 EntryIndex。在生成跳转指令时，可以通过 Switch::set_entry() 方法为一个 Switch 表格添加新的条目，其中每个条目都是一个基本块的 EntryIndex 和一个对应的标签（通常是一个整数）组成的元组。
...
collect_contiguous_case_ranges 方法用于收集所有连续的 case range，并将它们组合成一个 Vec，以便后续处理。

在 build_search_tree 方法中，首先通过 collect_contiguous_case_ranges 方法获取到所有连续的 case range，接着通过递归的方式，将这些连续的 case range 分解成更小的 case range。最终，每个 case range 都被组织成一个搜索树。这个搜索树是一个嵌套的结构，每个节点包含一个 case 和一个子搜索树。

在 build_jump_tables 方法中，根据搜索树，生成一个跳转表。跳转表的长度是 case range 的数量，每个 entry 是一个基本块。跳转表可以用于实现跳转语义，比如在 switch 表达式中跳转到对应的基本块。

总体来说，collect_contiguous_case_ranges 方法是用于帮助收集连续的 case range，方便后续的处理。通过递归的方式，将搜索树分解成一个个小的 case range，最终生成一个跳转表，实现了 switch 语义。
...
当连续case值形成的分组不超过3时,形成的块的数量较少,可以直接生成跳转代码, 用build_search_branches来实现此函数.
#+begin_src rust
...
当for的首次迭代结束后,  ~was_branch~ 被置为true. 也就是说 ~contiguous_case_ranges~ 剩下的每个组各自开辟一个新块.
...
这个 while 循环实际上是在模拟一个深度优先搜索过程。在每一次迭代中，从栈顶弹出当前节点的信息，如果连续 case 索引范围小于等于 3，则在当前块中生成对应的分支指令，否则根据范围拆分当前节点，并将生成的子节点信息压入栈顶，这样就可以迭代生成整个搜索树。最终生成的 cases_and_jt_blocks 向量包含了每个块的入口编号，以及每个 case 索引范围所对应的基本块和跳转表，这些信息将用于后续的跳转表生成过程。
...
不超过3时和上面的处理一样, 用 build_search_branches 生成代码
...
~build_search_tree~ 的输出是用于产生jt_table的信息.

所谓jt_table, 在生成ir的开头处声明: 
#+begin_src rust
...
值就是jt_table的索引,取值为 0,1,2,3, ...
例如值为1时,跳转到jt0的第二个目标 block3 中.
当值无法对应jt_table中的元素时,则跳转到落空块中(default分支).
...
此函数的输入: 就是为了生成jt_table准备的. 第二个Block是生成指令的起始块.
#+begin_src rust
 cases_and_jt_blocks: Vec<(EntryIndex, Block, Vec<Block>)>,
#+end_src
此函数的主体是一个for循环, 反向取出元素 (first_index从小到大)
#+begin_src rust
...
首先创建并初始化jump_table. 并将焦点移到此连续case值所对应的块
#+begin_src rust
...
然后要将val和各个分支的连续case值比较转化为 jump_table的索引.

例如: val=12, 一个case值形成的组是 [10,11,12,13]

为了生成正确的br_table指令,要将 val-10 作为该指令的操作数,即:
#+begin_src rust
br_table  [u32值](val - first_index), otherwise块, jump_table
#+end_src

但这个 first_index 的类型是 u128, 不能直接转为 Value所表示的值

**** TODO (val - first_index)的类型转换???
...
br_table() 是一个控制流指令，用于实现基于索引的跳转表。 它有两个操作数：一个是作为索引的值，另一个是指向一系列块的表。指令会根据索引跳转到表中的相应块。
#+begin_src rust
...
#+end_src
在上面的示例中，如果 selector 是 0，那么 jump_dest 将指向 block0。如果 selector 是 4，那么 jump_dest 将指向 block4。如果 selector 不在 0..=4 的范围内，则 jump_dest 将指向 default_block。
...
如何构建一个Function呢?
首先不必多说, ~Function::new()~ 创建出一个对象func. 函数在IR中被表示为控制流图, 因此要有基本块. 这可以通过func.dfg数据流图来创建出一个块: ~func.dfg.make_block()~.
...
块由一条条指令ins构成, 每条指令是一个表达式. 指令的添加也需要借助 ~FuncCursor~ 来完成, 添加完成一个表达式后,返回一个代表表达式结果的值value.
#+begin_src rust
  let value =  {
...
ssa的构建要借助SSABuilder. 使用ssa_builder首先要声明当前所操作的块:
#+begin_src rust
...
2. 将插入函数基本块中的指令所返回的"值Value"记录下来, 并用 ~def_var()~ 进行变量定义
#+begin_src rust
...
仍然是 declare_block + declare_block_predecessor
#+begin_src rust
...
**** block3
下面处理block3, 因为在block1中有 ~brnz y block3~ 因此block1是其前驱.并且是其唯一前驱.
...
****** 对当前块中每个未定义变量执行 ~predecessors_lookup()~
...
******* ~begin_predecessors_lookup()~ 将块的前驱压入调用栈
#+begin_src rust
...
******* ~run_state_machine()~ 开始

******** UseVar(): 在当前块中寻找,找不到则为前驱安排查找任务 
...
首先取出的元素是 ~Call::UseVar(block0)~, 先尝试从当前块block0中找到z的定义,找到终止此次循环.否则尝试用 ~use_var_nonlocal~ 从前驱中继续寻找.
...
下一次从calls中取出的是 ~Call::UseVar(block3)~, 这次不能在block3本身中找到z的定义,因此要借助 ~use_var_nonlocal~ 向calls中压入新任务, 用来到block3的前驱中寻找z的定义.

******** ~use_var_nonlocal()~ 将前驱作为任务压入调用栈
...
********* ~can_optimize_var_lookup()~: 只有一个前驱,且前驱不在环中

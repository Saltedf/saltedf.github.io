...
[[file:org-images/2023-03-03_15-39-33_screenshot.png]]
...
**** 要点: 
+ 被thread_local!包裹的变量必须是 *静态static* 的. 不能用mut修饰(可以用 ~RefCell~)
+ 每个线程有独立的值, 数据不共享.
+ 用 ~.with(|val| {..})~ 的方式取得值. 
...
#+attr_html: :width 320px
[[file:org-images/screenshot_2022-12-26_23-30-04.png]]
...
write_all(&[u8]) -> Result<()> 将buf中的全部字节写入,因此它也不返回字节数.
...
3. 从流中读取数据到buffer ~stream.read(&mut [a;N] )~
...
4. 发送数据 ~stream.write()~ 和 ~stream.flush()~
...
0. 将引用字段写成一个新的结构, 并真正own要指向的字段, 再让原结构包含这个新结构
1. Rc/Weak, Arc...
2. Pin 
...
*** Into<...>
    ...
    // into().raw()
    pub fn update<P:Into<Vector2f>>(&mut self,mouse_pos: P ) {
...
#+attr_html: :width 320px
[[file:org-images/screenshot_2022-12-26_23-20-21.png]]
...
这是因为在 x86_64 架构中，函数调用约定规定了 rdi、rsi、rdx、rcx、r8 和 r9 这几个寄存器用于函数参数的传递。因此，保存这些寄存器的值可以保证函数参数不会在协程切换时丢失。
